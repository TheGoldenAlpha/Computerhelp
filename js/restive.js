...
function rest_count(mixed_var, mode) {
    ...
    // *     example 1: rest_count([[0,0],[0,-4]], 'COUNT_RECURSIVE');
    // *     returns 1: 6
    // *     example 2: rest_count({'one' : [1,2,3,4,5]}, 'COUNT_RECURSIVE');
    // *     returns 2: 6
            ...
            case (mixed_var.hasOwnProperty(key)):
                ...
                switch(true)
                {
                    case (mode == 1 && mixed_var[key] && (isArray(mixed_var[key]) || isObject(mixed_var[key]))):
                        cnt += this.rest_count(mixed_var[key], 1);
                        break;
                }
                break;
        }
...
function arrayToInteger(str_arr)
{
    ...
    array_count_int = rest_count(str_arr);
...
function arrayHasDuplicates(arr) {
    var valuesSoFar = {},
        array_count_int = rest_count(arr);
...
function getClosestNumberMatchArray(haystack_arr, needle_int)
{
    ...
    for(var i = 0; i < rest_count(haystack_arr); i++)
    {
...
var Restive = (function(window, document, $) {
    ...
    window.rstv_store.storage = function(){
        var myArgs = Array.prototype.slice.call(arguments),
            key_str = myArgs[0],
            value_res = myArgs[1],
            is_value_valid_bool = !!((typeof value_res !== "undefined" && value_res !== null) && ((isString(value_res) && value_res != "") || isNumber(value_res) || (isArray(value_res) && rest_count(value_res) > 0) || isBool(value_res) || isObject(value_res))),
        ...
        try
        {
            ...
            switch(true)
            {
                case (is_value_null_bool):
                    ...
                    for (var i = 0; i < rest_count(list_del_key_arr); i++)
                    {
    ...
    function _toViewportBreakpoints(bp_arr, bp_class_arr)
    {
        try{
            ...
            var counter_int = 0,
                ...
                bp_arr_count_int = rest_count(bp_arr),
                bp_class_arr_count_int = rest_count(bp_class_arr),
                bp_attrib_arr_count_int = rest_count(bp_attrib_arr),
            ...
            var bp_temp_w_sort_arr_size_int = rest_count(bp_temp_w_sort_arr);
    ...
    init : function(options){
      ...
      try{
                ...
                case (in_array('init', callback_type_arr)):
                    switch(true)
                    {
                        case (in_array('init', callback_type_arr)):
                            ...
                            for(var i = 0; i < rest_count(func_name_arr); i++)
                            {
        ...
        setBreakpoints: function(){
            ...
            var bp_width_arr_count_int = rest_count(bp_width_arr);
                        ...
                        case (options.turbo_classes_reflow && isString(options.turbo_classes) && options.turbo_classes != ''):
                            ...
                            switch(true)
                            {
                                case (opt_isset_is_mobile_bool):
                                    ...
                                    for(var j = 0; j < rest_count(turbo_classes_arr); j++)
                                    {
        ...
        _addTurboClasses: function(class_data_str, opt_turbo_classes){
            ...
            for(var i = 0; i < rest_count(opt_pc_arr); i++)
            {
