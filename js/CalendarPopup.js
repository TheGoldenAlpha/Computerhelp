...
5) The calendar display uses "graphic/dot_black.gif" which is a 1x1 gray pixel of
...
function CalendarPopup() {
        ...
        c.monthNames = new Array("Januar","Februar","März","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember");
        c.monthAbbreviations = new Array("Jan","Feb","Mär","Apr","Mai","Jun","Jul","Aug","Sep","Okt","Nov","Dez");
		...
		window.CalendarPopup_url = null;
		window.CalendarPopup_divname = null;
        // Method mappings
...
function CalendarPopup_tmpReturnFunction(y,m,d) {
        if (window.CalendarPopup_targetInput!=null) {
				...
				if(window.CalendarPopup_url != null){
				document.getElementById("calendardiv").style.visibility = "hidden";
				window.location.href = window.CalendarPopup_url+"&datum="+formatDate(d,window.CalendarPopup_dateFormat);
				}else if(window.CalendarPopup_divname != null){
				window.CalendarPopup_targetInput.value = formatDate(d,window.CalendarPopup_dateFormat);
				document.getElementById(window.CalendarPopup_divname).style.visibility = "hidden";
				}
				
/*				if(window.CalendarPopup_url != null){
				window.location.href = "?datum="+formatDate(d,window.CalendarPopup_dateFormat)+"&usersichtung="+window.CalendarPopup_url+"&ansicht=aendern";
				}else if(window.CalendarPopup_divname != null){
				window.CalendarPopup_targetInput.value = formatDate(d,window.CalendarPopup_dateFormat);
				document.getElementById(window.CalendarPopup_divname).style.visibility = "hidden";
				}else{
				window.location.href = "?datum="+formatDate(d,window.CalendarPopup_dateFormat);
				}*/
/*alt					if(window.CalendarPopup_targetInput.name == "datum" || window.CalendarPopup_targetInput.name == "von" || window.CalendarPopup_targetInput.name == "bis"){
                window.CalendarPopup_targetInput.value = formatDate(d,window.CalendarPopup_dateFormat);
				}else if(window.CalendarPopup_url != null){
				window.location.href = "?datum="+formatDate(d,window.CalendarPopup_dateFormat)+"&usersichtung="+window.CalendarPopup_url+"&ansicht=aendern";
				}else{
				window.location.href = "?datum="+formatDate(d,window.CalendarPopup_dateFormat);
				}
...
function CalendarPopup_select(inputobj, linkname, format, url, divname, YourDate) {
		...
		if(url){
        window.CalendarPopup_url = url;
		}
		if(divname){
        window.CalendarPopup_divname = divname;
		}else{
		window.CalendarPopup_divname = "calendardiv";
		}
		...
		if(YourDate){
		 ...
		 mytempDate = inputobj.value;
		}

        if (mytempDate!="") {
                var time = getDateFromFormat(mytempDate,format);
...
function CalendarPopup_getStyles() {
        var result = "";
        result += "<STYLE>\n";
        result += "TD.cal { font-family:arial; font-size: 10pt; color:#ffffff; }\n";
		result += "TD.calday { font-family:arial; font-size: 10pt; color:#000000; background-color:#EFEFEF; border-width:1px; border-type:solid; border-color:#800000;}\n";
        result += "TD.calmonth { font-family:arial; font-size: 10pt; text-align:right; }\n";
        result += "TD.caltoday { font-family:arial; font-size: 10pt; text-align:right; color:#ffffff; background-color:#66CCFF; border-width:1; border-type:solid; border-color:#800000; }\n";
        result += "A.textlink { font-family:arial; font-size: 10pt; height: 20px; color: black; }\n";
        result += ".disabledtextlink { font-family:arial; font-size: 10pt; height: 20px; color: #808080; }\n";
...
function CalendarPopup_getCalendar() {
        ...
        if (this.type == "WINDOW") {
                ...
                result += '<CENTER><TABLE WIDTH=100% BORDER=0 BORDERWIDTH=0 CELLSPACING=0 CELLPADDING=0 BGCOLOR=#ffffff >\n';
                }
        else {
                result += '<TABLE WIDTH=144 BORDER=1 BORDERWIDTH=1 BORDERCOLOR="#808080" CELLSPACING=0 CELLPADDING=1 BGCOLOR=#ffffff >\n';
        ...
        if (this.displayType=="date" || this.displayType=="week-end") {
                ...
                result += '<TR BGCOLOR="#0BA3E2">\n';
                var refresh = 'javascript:'+windowref+'CalendarPopup_refreshCalendar';
                if (this.isShowYearNavigation) {
                        var td = '<TD BGCOLOR="#0BA3E2" CLASS="cal" ALIGN=CENTER VALIGN=MIDDLE ';
                        ...
                        result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=22 ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="'+refresh+'('+this.index+','+last_month+','+last_month_year+');">&lt;&lt;</A></B></TD>\n';
                        result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=100 ALIGN=CENTER>'+this.monthNames[month-1]+' '+year+'</TD>\n';
                        result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=22 ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="'+refresh+'('+this.index+','+next_month+','+next_month_year+');">&gt;&gt;</A></B></TD>\n';
                        }
                result += '</TR></TABLE>\n';
                result += '<TABLE WIDTH=140 BORDER=0 CELLSPACING=0 CELLPADDING=0 ALIGN=CENTER  BGCOLOR=#ffffff class="rahmen" >\n';
                result += '<TR>\n';
                var td = '<TD CLASS="calday" ALIGN="RIGHT" WIDTH="50">';
                for (var j=0; j<7; j++) {
						result += td+this.dayHeaders[(this.weekStartDay+j)%7]+'</TD>\n';
                        }
                result += '</TR>\n';
                result += '<TR><TD COLSPAN=7 ALIGN=CENTER><IMG SRC="graphic/dot_black.gif" WIDTH=140 HEIGHT=1></TD></TR>\n';
                ...
                if (this.disabledWeekDays[current_weekday+1]) {
                        ...
                        dasatcalender = "&nbsp;<a  CLASS=\"text\" href=\"#\" onClick=\"document.getElementById('"+window.CalendarPopup_divname+"').style.visibility='hidden';\">schliessen</a>\n";
                        result += dasatcalender;;// '                <A CLASS="textlink" HREF="javascript:'+windowref+this.returnFunction+'(\''+now.getFullYear()+'\',\''+(now.getMonth()+1)+'\',\''+now.getDate()+'\');'+windowref+'CalendarPopup_hideCalendar(\''+this.index+'\');">'+this.todayText+'</A>'+dasatcalender+'\n';
                        }
                result += '                <BR>\n';
        ...
        if (this.displayType=="month" || this.displayType=="quarter" || this.displayType=="year") {
                ...
                if (this.displayType!="year" && this.isShowYearNavigation) {
                        ...
                        result += '<TR BGCOLOR="#0BA3E2">\n';
                        result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=22 ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="javascript:'+windowref+'CalendarPopup_refreshCalendar('+this.index+','+(year-1)+');">&lt;&lt;</A></B></TD>\n';
                        result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=100 ALIGN=CENTER>'+year+'</TD>\n';
                        result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=22 ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="javascript:'+windowref+'CalendarPopup_refreshCalendar('+this.index+','+(year+1)+');">&gt;&gt;</A></B></TD>\n';
                        result += '</TR></TABLE>\n';
                        }
                }
        ...
        if (this.displayType=="year") {
                ...
                result += '<TABLE WIDTH=144 BORDER=0 BORDERWIDTH=0 CELLSPACING=0 CELLPADDING=0>\n';
                result += '<TR BGCOLOR="#0BA3E2">\n';
                result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=50% ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="javascript:'+windowref+'CalendarPopup_refreshCalendar('+this.index+','+(year-(yearColumnSize*2))+');">&lt;&lt;</A></B></TD>\n';
                result += '        <TD BGCOLOR="#0BA3E2" CLASS="cal" WIDTH=50% ALIGN=CENTER VALIGN=MIDDLE><B><A CLASS="cal" HREF="javascript:'+windowref+'CalendarPopup_refreshCalendar('+this.index+','+(year+(yearColumnSize*2))+');">&gt;&gt;</A></B></TD>\n';
                result += '</TR></TABLE>\n';
                result += '<TABLE WIDTH=120 BORDER=0 CELLSPACING=1 CELLPADDING=0 ALIGN=CENTER>\n';
